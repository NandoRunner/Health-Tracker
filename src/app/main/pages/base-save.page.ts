...
export class BaseSavePage<T> implements OnInit {
  ...
  protected init(itemId: any): void {
    ...
    this.service
      .get(itemId)
      .pipe(take(1))
      .subscribe(({ date, value, value2, value3, valueMax, valueMin, heartRate }) => {
        this.formGroup.get('id').setValue(itemId);
        this.formGroup.get('date').setValue(date.toDate().toISOString());
        this.formGroup.get('value').setValue(valueMax == null ? value : valueMax);
        this.formGroup.get('value2').setValue(valueMin == null ? value2 : valueMin);
        this.formGroup.get('value3').setValue(heartRate == null ? value3 : heartRate);
      });
  }
